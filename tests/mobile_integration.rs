...
//! 
//! This file demonstrates key patterns for mobile app integration:
...
fn test_mobile_lifecycle() {
    ...
    // Simulate connection
    let server_sk = ring_server.derive_device_x25519("server-kid", b"server-device", 3).unwrap();
    let server_static_pk = pubky_noise::kdf::x25519_pk_from_sk(&server_sk);
    
    // Handshake
    let client_ref = manager.client().unwrap();
    let (c_link, _used_epoch, first_msg) = client_start_ik_direct(
        client_ref,
        &server_static_pk,
        3,
        None
    ...
    let saved_state = manager.save_state(&session_id).unwrap();
    ...
    let mut new_manager = NoiseManager::new_client(
        manager.client().unwrap().clone(),
    ...
    assert_eq!(new_manager.get_status(&session_id), Some(ConnectionStatus::Connected));
}
...
fn test_session_id_serialization() {
    ...
    let (c_link, _, first_msg) = client_start_ik_direct(&client, &server_static_pk, 3, None).unwrap();
    let (_s_link, _) = server_accept_ik(&server, &first_msg).unwrap();
...
fn test_thread_safe_manager() {
    ...
    let server_sk = server_ring.derive_device_x25519("server", b"server", 3).unwrap();
    let server_static_pk = pubky_noise::kdf::x25519_pk_from_sk(&server_sk);
    
    let (link, _, _) = client_start_ik_direct(&client, &server_static_pk, 3, None).unwrap();
...
fn test_error_codes() {
    ...
    assert_eq!(Decryption("test".to_string()).code(), NoiseErrorCode::Decryption);
...
fn test_streaming_for_mobile() {
    ...
    let (c_link, _, first_msg) = client_start_ik_direct(&client, &server_static_pk, 3, None).unwrap();
    let (s_link, _) = server_accept_ik(&server, &first_msg).unwrap();
...
fn test_multiple_session_management() {
    ...
    let server1_sk = ring_server1.derive_device_x25519("server1", b"server1", 3).unwrap();
    let server1_pk = pubky_noise::kdf::x25519_pk_from_sk(&server1_sk);
    
    let server2_sk = ring_server2.derive_device_x25519("server2", b"server2", 3).unwrap();
    let server2_pk = pubky_noise::kdf::x25519_pk_from_sk(&server2_sk);
    
    let (link1, _, _) = client_start_ik_direct(&client, &server1_pk, 3, None).unwrap();
    let (link2, _, _) = client_start_ik_direct(&client, &server2_pk, 3, None).unwrap();
    
    let sid1 = link1.session_id().clone();
    let sid2 = link2.session_id().clone();
    
    manager.add_session(sid1.clone(), link1);
    manager.add_session(sid2.clone(), link2);
...
fn test_connection_status_tracking() {
    ...
    let server_sk = ring_server.derive_device_x25519("server", b"server", 3).unwrap();
    let server_pk = pubky_noise::kdf::x25519_pk_from_sk(&server_sk);
    
    let (link, _, _) = client_start_ik_direct(&client, &server_pk, 3, None).unwrap();
    let session_id = link.session_id().clone();
    
    manager.add_session(session_id.clone(), link);
    
    // Initial status should be Connected
    assert_eq!(manager.get_status(&session_id), Some(ConnectionStatus::Connected));
    
    // Update status (simulate network change)
    manager.set_status(&session_id, ConnectionStatus::Disconnected);
    assert_eq!(manager.get_status(&session_id), Some(ConnectionStatus::Disconnected));
    
    manager.set_status(&session_id, ConnectionStatus::Reconnecting);
    assert_eq!(manager.get_status(&session_id), Some(ConnectionStatus::Reconnecting));
    
    manager.set_status(&session_id, ConnectionStatus::Error);
    assert_eq!(manager.get_status(&session_id), Some(ConnectionStatus::Error));
}
