...
fn test_multiple_session_management() {
    ...
    let (sid1_temp, first_msg1) = manager.initiate_connection(&server1_pk, None).unwrap();
    ...
    let (sid2_temp, first_msg2) = manager.initiate_connection(&server2_pk, None).unwrap();
